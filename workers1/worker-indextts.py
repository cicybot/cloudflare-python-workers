...
# 处理单个任务（带详细日志）
# --------------------
def run_indextts_task(task_data):
    # 只处理 IndexTTS 类型的任务
    task_type = task_data.get("type", "index-tts")  # 向后兼容，默认认为是 index-tts
    if task_type != "index-tts":
        return
    ...
    while True:
        ...
        for tid in task_ids:
            ...
            if task_data_str:
                ...
                if task_data.get("type", "index-tts") == "index-tts":
                    run_indextts_task(task_data)
